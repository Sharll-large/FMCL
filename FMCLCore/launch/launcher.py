...
def launch(game_directory: str, version_name: str, account: dict,
    ...
    cmdlist = ["cd"]
    work_dir = pathlib.Path(game_directory)
    if standalone:
        work_dir /= "versions"
        work_dir /= version_name
    cmdlist.append(str(work_dir.resolve()))
    cmdlist.append("&&")
    ...
    if standalone:
        game_directory = verpath

    if not (os.path.exists(game_directory) and os.path.exists(jsonpath) and version_name != ""):
        return False
